...
from odoo.addons.product_set.tests.common import ProductSetCommon
...
class TestProductSet(ProductSetCommon):
    ...
    def test_add_set_on_empty_so(self):
        so = self.so_model.create({"partner_id": self.partner_1.id})
    ...
    def test_add_set_non_matching_partner(self):
        so = self.so_model.create({"partner_id": self.partner_1.id})
        self.product_set.partner_id = self.partner_2.id
        wiz = self._get_wiz(order_id=so.id)
    ...
    def test_add_set_non_matching_partner_ctx_override(self):
        ...
        self.assertEqual(len(so.order_line), 0)
        wiz = self._get_wiz(order_id=so.id).with_context(
            allowed_order_partner_ids=[self.partner_2.id]
        )
        wiz.add_set()
        self.assertEqual(len(so.order_line), 3)
    ...
    def test_discount(self):
        ...
        prod_set = self.env["product.set"].create(
            {"name": "Test", "set_line_ids": [Command.link(set_line.id)]}
        )
        so = self.so
        ...
        wizard.write(
            {
                "product_set_line_ids": [
                    Command.unlink(wizard.product_set_line_ids[0].id)
                ]
            }
        )
    ...
    def test_add_set_section(self):
        base_line_ids = self.so.order_line
        count_lines = len(self.so.order_line)
    ...
    def test_product_set_toggle_active(self):
        ...
        self.product_set.partner_id = self.partner_1
        self.product_set_2.partner_id = self.partner_1
        self.product_set_3.partner_id = self.partner_2
